...
class Portal::PortalBatSend < Portal::PortalBatBase
  ...
  def main
    rows = []
    @not_portal_list = []  #//インスタンス変数を作成 katou
    ...
    ###katou_ry
    #保安点検マスタ1と3でそれぞれ調査日を降順で1件取得したidと合致するレコードを
    #販売店コードと消費先コード毎に抽出
    ###katou_ry
    ...
    ###katou_ry
    #顧客マスタにそれぞれ保安点検マスタ1と3を顧客コードと消費先コードをLeftJoinし、
    #最終送信日がNULLまたは最終送信日が更新日付より過去日のものを対象
    ###katou_ry
    ...
    kokya1_db_search.each do |dbs|
      if all_hankaku(dbs.TEL_TEL.to_s.slice(-4,4).to_s) && !(dbs.TEL_TEL.to_s.slice(-4,4).to_s=~ /\p{katakana}/)
       ...
       ###katou_ry
       # String TEL_TEL = "09026337447"
       # p TEL_TEL.to_s.slice(-4,4)
       # 後方から4文字目から4文字を取得的な?
       #半角で/\p{Katakana}/の正規表現にマッチするものは対象外にしてる?
       ###katou_ry
